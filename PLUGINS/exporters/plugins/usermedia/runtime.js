...
(function ()
{
	...
	if (!supportsMediaDevices && !navigator["getUserMedia"])
	{
		// look for prefixed versions of the old API
	...
	{
		return !!navigator["getUserMedia"] || supportsMediaDevices;
	};
	...
	{
		if (isRequesting || !(navigator["getUserMedia"] || supportsMediaDevices))
		...
		{
			// Request to use the given video device ID. Note in a previous spec this was the "sourceId" and "id".
			if (supportsMediaDevices)
			{
				constraints["video"] = {
					"optional": [{
						"deviceId": videoSources[source_index]["deviceId"]
					}]
				};
			}
			else
		...
		{
			navigator["getUserMedia"](constraints, onGotMediaStream, onError);
		}
	};
	...
	Acts.prototype.RequestMic = function (tag, source_index)
	{
		...
		{
			// Request to use the given audio device ID. Note in a previous spec this was the "sourceId" and "id".
			if (supportsMediaDevices)
			{
				constraints["audio"] = {
					"optional": [{
