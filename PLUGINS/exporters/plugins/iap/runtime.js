	...
	*/
	...
	function CordovaiOSStore()
	{
		...
		this.iap["init"]({
			...
			"ready": function ()
			{
				// load after 100ms to ensure product ids have had a chance to be set
				window.setTimeout(function ()
				{
					self.iap["load"](self.product_id_list, function (productIds, invalidIds)
					{
						self.loaded = true;
						
						for (var i = 0; i < productIds.length; ++i)
						{
							// productIds[i] = { "id", "title", "description", "price" };
							self.product_info[productIds[i]["id"]] = productIds[i];
						}
						
						if (self.onstorelistingsuccess)
							self.onstorelistingsuccess();
						
						// restore purchases so known purchases are made available again
						self.iap["restore"]();
					});
				}, 100);
			},
			...
			{
				if (isTestMode)
					alert("IAP error (only reported in test mode): " + errorText + " (code: " + errorCode + ")");
				
				if (self.onpurchasefail)
					self.onpurchasefail(self.lastProductId, errorText);
			},
			"purchase": function (transactionId, productId)
			{
				self.owned_products[productId] = true;
			...
			{
				if (isTestMode)
					alert("IAP error (only reported in test mode): restore failed with code " + errorCode);
			}
		});
	};
	...
	{
		// no-op - no way to do this with the given API - let load() fire the store listing events
	};
	...
	instanceProto.onCreate = function()
	{
		...
		{
			if (this.runtime.isiOS && window["storekit"])
			{
